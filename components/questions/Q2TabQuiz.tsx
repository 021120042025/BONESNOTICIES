...
export default function Q2TabQuiz({ value, onChange }: Props) {
  ...
  function handleSelect(option: string) {
    ...
    onChange(next);
    const nextUnanswered = Q2_MINI.findIndex((_, i) => i > active && next[i] === undefined);
    if (nextUnanswered !== -1) {
      setTimeout(() => setActive(nextUnanswered), 350);
    }
  }

  const mini = Q2_MINI[active];
    ...
    <div className="space-y-3">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="bg-ink px-4 py-3 flex items-center gap-2">
              <span className="text-2xl">{INSTITUTION_ICONS[mini.image]}</span>
              <span className="font-sans text-[11px] font-bold uppercase tracking-widest text-bone/70">
                Institució {active + 1} de {Q2_MINI.length}
              </span>
            </div>
      ...
      <div className="flex justify-between items-center pt-1">
        ...
        <span className="font-sans text-xs font-medium text-ink/40 tabular-nums">
          {active + 1} / {Q2_MINI.length}
        </span>
        <button
          onClick={() => setActive(Math.min(Q2_MINI.length - 1, active + 1))}
          disabled={active === Q2_MINI.length - 1}
          className="font-sans text-xs font-medium text-ink/50 disabled:opacity-30 no-select"
        >
          Següent →
        </button>
